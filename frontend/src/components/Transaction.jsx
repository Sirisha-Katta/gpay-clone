...
function Transaction() {
  ...
  const handleSendMoney = async () => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:8000/send-money/", {
      ...
      const userId = localStorage.getItem("user_id");
      const balanceResponse = await axios.get(`http://localhost:8000/user/${userId}`);
  ...
  const handleAddLabel = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:8000/create-label/", {
