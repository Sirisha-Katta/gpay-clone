...
const Report = () => {
  ...
  useEffect(() => {
    const fetchReportData = async () => {
      ...
      if (userId) {
        try {
          const response = await axios.get(`http://localhost:8000/user/${userId}/transactions`);
    ...
    const fetchSavedDetails = async () => {
      ...
      if (userId) {
        try {
          const response = await axios.get(`http://localhost:8000/user/${userId}/preferences`);
  ...
  const handleSaveDetails = async () => {
    ...
    if (userId) {
      try {
        const response = await axios.post(`http://localhost:8000/user/${userId}/preferences`, {
          income,
          preferences,
        });
        setSavedDetails(response.data.preferences);
